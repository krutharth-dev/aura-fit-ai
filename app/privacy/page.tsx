...
export default function PrivacyPage() {
  return <main className="policy-shell">
    ...
    <article className="policy-card">
      <p className="policy-kicker">EFFECTIVE 19 SEPTEMBER 2026</p>
      ...
      <p>For signed-in users, we store chat titles, messages, coach responses, timestamps, the fitness profile fields you choose to save, and workout journal entries (session names, dates, duration, exercises, sets, reps, weights and notes). Session records contain a hash of the random session token, its account relationship and expiry time. The raw session token exists only in an HttpOnly browser cookie.</p>

      <h2>2. Account isolation</h2>
      <p>Every workout-journal and saved-conversation read, write, rename and deletion is authorised on the server using the signed-in account ownership key. A different account cannot retrieve a conversation by guessing or supplying its identifier. Signing out invalidates the active server session and expires the browser cookie.</p>
      ...
      <p>You may use the coach without signing in, in which case no durable conversation history is created. Signed-in users can review and delete individual conversations from the sidebar and can remove their saved training profile. In the training journal you can export all workouts as JSON or permanently delete individual sessions.</p>

...
export default function TrainingClient({ signedIn, name, signInPath }: { signedIn: boolean; name: string; signInPath: string }) {
  ...
  return <main className="training-shell">
    <header className="training-header"><Link className="training-brand" href="/">AURA<span>FIT</span></Link><nav aria-label="Workspace"><Link href="/">AI coach</Link><Link href="/training" aria-current="page">Training journal</Link></nav><span className="training-account">{signedIn ? name.split(" ")[0] : <a href={signInPath}>Sign in</a>}</span></header>

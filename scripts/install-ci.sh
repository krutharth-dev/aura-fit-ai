...
runtime_root="${AURA_FIT_RUNTIME_ROOT}"
expected_cache="${runtime_root}/npm-cache"

echo "[aura-fit] validating writable install environment"
...
touch "${expected_cache}/.aura-fit-write-test"
rm -f "${expected_cache}/.aura-fit-write-test"
echo "[aura-fit] environment passed: cache=${expected_cache}"

lock_file="${runtime_root}/install.lock"
exec 9>"${lock_file}"
if ! flock -n 9; then
  echo "Another dependency install is already running for ${AURA_FIT_PROJECT_ROOT}." >&2
  exit 75
fi
...
for process in /proc/[0-9]*; do
  ...
  [[ "${process_cwd}" == "${AURA_FIT_PROJECT_ROOT}" ]] || continue
  process_command="$(tr '\0' ' ' <"${process}/cmdline" 2>/dev/null || true)"
  if [[ "${process_command}" == *"npm ci"* ]]; then
    echo "Another npm ci is visible in ${AURA_FIT_PROJECT_ROOT}; refusing to overlap installs." >&2
    exit 75
  fi
done

lockfile_sha256="$(sha256sum "${AURA_FIT_PROJECT_ROOT}/package-lock.json" | awk '{print $1}')"
use_seeded_cache=0
seed_cache="${AURA_FIT_NPM_CACHE_SEED:-}"
if [[ -n "${seed_cache}" && -d "${seed_cache}" ]]; then
  seed_lockfile_sha256="$(cat "${seed_cache}/.aura-fit-lockfile-sha256" 2>/dev/null || true)"
  if [[ "${seed_lockfile_sha256}" == "${lockfile_sha256}" ]]; then
    echo "[aura-fit] restoring image-seeded npm cache"
    cp -a "${seed_cache}/." "${expected_cache}/"
    use_seeded_cache=1
    echo "[aura-fit] image cache seed matched; registry fallback remains available"
  else
    echo "[aura-fit] image cache seed does not match this lockfile; using the network path"
  fi
fi

locked_vinext_output="$({ node --input-type=module - "${AURA_FIT_PROJECT_ROOT}/package-lock.json" <<'NODE'
...
NODE
  ...
  echo "[aura-fit] downloading the complete locked vinext tarball"
  curl \
    --fail \
    --location \
    --silent \
  ...
  echo "[aura-fit] verifying locked vinext tarball integrity"
...
NODE
  echo "[aura-fit] network and integrity preflight passed"
fi

echo "[aura-fit] running exactly one bounded npm ci"
...
timeout \
  --signal=TERM \
  --kill-after="${AURA_FIT_INSTALL_KILL_AFTER:-15s}" \
  "${AURA_FIT_INSTALL_TIMEOUT:-8m}" \
  npm "${npm_ci_args[@]}"
...
node --input-type=module - "${AURA_FIT_PROJECT_ROOT}/node_modules/.aura-fit-install.json" "${lockfile_sha256}" <<'NODE'
...
NODE
echo "[aura-fit] npm ci passed and vinext is available"
